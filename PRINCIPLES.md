...
堆肥区要定期清理，没长出芽的废料直接丢。不然堆肥桶也会变成垃圾堆。

- raw/ 素材 90 天未被编译 → 归档，不再处理
- compiled/ 草稿 180 天未被审核 → 自动拒绝
- 知识的价值会随时间衰减，长期无人问津的内容不该继续占用空间和注意力
- 半衰期不是惩罚，是自然代谢 — 真正重要的知识不会被遗忘，因为它会被反复使用和引用

### Minimum Viable Knowledge（最小可用知识）

Wiki 入库应该是"最小可用知识"——只有高频精准调用的才入库，模糊的、变动大的留在堆肥区让 Agent 搜。

- 这个原则反的是"入库强迫症"——不是所有内容都值得入库
- 入库标准：能否被精准调用？是否稳定（不会频繁变动）？是否有明确的边界？
- 模糊的、还在探索中的、频繁变动的知识 → 留在 compiled/ 或 raw/，Agent 可以通过 #ask 搜到
- 只有"我想下次问的时候一定能得到这个答案"的知识 → 才进 wiki/
- 入库门槛高，但一旦入库就是可信的
